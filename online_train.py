...
def main(args):
    ...
    def optimize(obs, act, bound):
        ...
        for obs, act in tqdm.tqdm(loader, total=len(loader), position=1):
    ...
    with Executor(max_workers=1) as executor:
        env = executor.submit(utils.env.make_env, seed).result()
        ...
        progress = tqdm.trange(epochs, desc='grip=?, move=?, success=?', position=0)
        for e in progress:
            epoch = e + 1
            if args.method == 'dagger':
                expert.β = p ** epoch
            ...
            if epoch % save_period == 0:
                net.save('storage/models/online/resnet18_{}.pth'.format(epoch))
        net.save('storage/models/online/resnet18_{}.pth'.format('current'))
...
def tensorize(trajectories):
    ...
    trajectory_bound = []
    offset = 0
    for (obs_list, act_list) in trajectories:
